...
window.onload = function () {
    ...
    var tags = ["Imou", "Hikvision", "Ezviz", "Kbvision", "Dahua",];
...
function phanTich_URL_chiTietSanPham() {
    ...
    var s = addThongSo('Độ phân giải và tốc độ ghi hình', sanPhamHienTai.detail.screen);
    s += addThongSo('Tiện ích', sanPhamHienTai.detail.os);
    s += addThongSo('Kích thước', sanPhamHienTai.detail.camara);
    s += addThongSo('Băng tần WiFi', sanPhamHienTai.detail.camaraFront);
    s += addThongSo('Phần mềm', sanPhamHienTai.detail.cpu);
    s += addThongSo('Tầm nhìn xa hồng ngoại', sanPhamHienTai.detail.ram);
    s += addThongSo('Lưu trữ và nguồn điện', sanPhamHienTai.detail.rom);
    s += addThongSo('Cảm biến hình ảnh', sanPhamHienTai.detail.microUSB);
    s += addThongSo('Chuẩn chống nước', sanPhamHienTai.detail.battery);
    info.innerHTML = s;
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        case 'tragop':
            var span = `<span style="font-weight: bold">Khách hàng được tặng nguồn và 25m dây điện `+ `</span>`;
            return span;
